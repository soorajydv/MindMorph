...
const signupController = (request, response) => {
  ...
  try {
    const result = pool.query('SELECT * FROM users WHERE username = $1 OR email = $2', [username, email], (error, result) => {
      if (error) {
        throw error;
...
const loginController = async (request, response) => {
  ...
  try {
    ...
    .then(async (hash) => {
    const result = pool.query('SELECT * FROM users WHERE email = $1 OR password = $2', [email, hash], (error, result) => {
      if (error) {
        throw error;
...
/*  Google AUTH  */
const GOOGLE_CLIENT_ID = '894514061627-oss0rhjl80ucj4lr0avoh63r97vcrjod.apps.googleusercontent.com';
...
passport.use(new GoogleStrategy({
  ...
  pool.query('SELECT * FROM o_auth_user WHERE oauth_id = $1 AND   oauth_source = $2', [id, provider], (error, result) => {
    if (error) {
      throw error;
    } else if (result.rows.length === 0) {
      pool.query(
